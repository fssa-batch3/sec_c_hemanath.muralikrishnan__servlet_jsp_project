...
function show_indv(obj) {
  ...
  indv_farmer_img.setAttribute(
    "alt",
    `image of farmer ${item.farmer.image.alt}`
  );
...
const rel_products = product_details.filter((rel) => {
  if (rel.category.id === product_cat && rel.status && product_id !== rel.id) {
    return rel;
  }
  return false;
});
